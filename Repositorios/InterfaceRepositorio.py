...
class InterfaceRepositorio(Generic[T]):
    def __init__(self):
        ...
        dataConfig = self.loadFileConfig()  #datos de configuration
        client = pymongo.MongoClient(dataConfig["db-connection"], tlsCAFile=ca)  #cliente conectado a la db
        self.baseDatos = client[dataConfig["db-name"]] #Base de datos
        claseGenerica = get_args(self.__orig_bases__[0])  #ipo especifico de T en ejecucion
